...
public class PlayScreen implements Screen {
	...
	private void createCreatures(StuffFactory factory) {
		...
		for (int z = 0; z < world.depth(); z++) {
                    //увеличено количество грибов на уровне
			for (int i = 0; i < 4; i++) {
				factory.newFungus(z);
			}
			for (int i = 0; i < 10; i++) {
	...
	public void displayOutput(AsciiPanel terminal) {
		...
		//String hunger = String.format("%s", hunger());
                String hunger = String.format("%d/%d", player.food(), player.maxFood());
                String hunger_state = String.format("%s", hunger());
                int max_xp = (int) (Math.pow(player.level(), 1.75) * 25);
                String xp = String.format("%d/%d xp", player.xp(), max_xp);
		...
		if (hunger_state.equals("Hungry"))
			hungerColor = Color.YELLOW;
		else if (hunger_state.equals("Starving"))
		...
		terminal.write(manareg, 71, 12);
                ...
                //добавлен опыт
                terminal.write("Experience:" + xp, 71, 13);
                int level = player.z + 1;
                terminal.write("Dungeon level:" + level, 71, 14);
                //строка изменена с 13 на 14
		terminal.write("============================", 71, 15);
                //с 14 на 15
		terminal.write("Effects:", 71, 16);
                //с 15 на 16
		int x = 16;

		if (player.effects().size() == 0)
                    //с 15 на 16
			terminal.write("None", 71, 17);
		else {
