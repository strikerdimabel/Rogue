...
public class CreatureAi {
	...
	protected Item getWeaponToThrow() {
		...
		for (Item item : creature.inventory().getItems()) {
			...
			if (toThrow == null
					|| item.thrownAttackValue() > toThrow.attackValue())
				toThrow = item;
		}
	...
	protected boolean canUseBetterEquipment() {
		int currentWeaponRating = creature.weapon() == null ? 0 : creature
				.weapon().attackValue() + creature.weapon().rangedAttackValue();
		int currentArmorRating = creature.armor() == null ? 0 : creature
				.armor().defenseValue();
		...
		for (Item item : creature.inventory().getItems()) {
			...
			boolean isArmor = item.attackValue() + item.rangedAttackValue() < item
					.defenseValue();
